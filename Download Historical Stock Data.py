...
avg_returns_per_cluster = returns_cluster.groupby(["Year", "Cluster"])["Return"].mean().reset_index()

# Compute the covariance and correlation matrix between clusters
avg_cluster_returns_wide = avg_returns_per_cluster.pivot(index="Year", columns="Cluster", values="Return")
...
for cluster_num in range(1, num_clusters + 1):
    ...
    print(f"Cluster {cluster_num} (ESG: {avg_esg_per_cluster[cluster_num]:.2f}): {', '.join(cluster_members)}")
...
plt.title(f"Dendrogram for alpha = {alpha} with linkage: " + linkage_method + 
