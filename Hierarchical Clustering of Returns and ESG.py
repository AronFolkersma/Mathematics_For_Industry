...
def corr_metric(returns_df, p):
    ...
    if (p == 0):
        ...
        return corr_dist_matrix/np.max(corr_dist_matrix)
...
def compute_clusters(alpha, num_clusters, print_ = True, annual_returns_dict = annual_returns_dict, ESG_normalized = ESG_normalized):
...
def plot_dendrograms(linkage_matrix, alpha, num_clusters):
...
def parallel_coordinates_plot(num_clusters_list, alpha_list):
    results = []
    
    for num_clusters in num_clusters_list:
        for alpha in alpha_list:
            compute_clusters(alpha, num_clusters, False)
    ...
    fig = go.Figure(data=
        go.Parcoords(
            line = dict(color = results_df["Alpha"],
                       colorscale = "turbo",
                       showscale = True,
                       cmin = 0,
                       cmax = 1,
            ...
            dimensions = list([
                ...
                dict(range = [results_df["ESG score"].min(), 
                              results_df["ESG score"].max()],
                     label = "ESG score", values = results_df["ESG score"]),
                ...
                dict(range = [results_df["Alpha"].min(), 
                              results_df["Alpha"].max()],
                     label = "Alpha", values = results_df["Alpha"])])
        )
    )
    fig.update_layout(
        title=("Portfolio with risk aversion " + str(risk_aversion) + 
               " and ESG constraint " + str(ESG_constraint) + 
               " (Alpha: 0 is only correlation, 1 is only ESG)"),
    ...
    # fig.write_html("High_risk_aversion.html")
...
global risk_aversion; risk_aversion = 12  # Adjust risk aversion (low value is high return/high risk and high value low return/low risk)
global ESG_constraint; ESG_constraint = 18  # Adjust ESG constraint (weighted ESG score must be lower than the constraint)

global p; p = 0 # Choose metric
global linkage_method; linkage_method = "ward"  # Choose "single", "complete", "average", "ward", etc.
alpha = 0.15
num_clusters = 20 # Choose number of clusters

compute_clusters(alpha, num_clusters)
plot_dendrograms(linkage_matrix, alpha, num_clusters)
plot_heatmaps()
output = optimize_portfolio(avg_return_per_cluster, cluster_cov_matrix, avg_esg_per_cluster)
...
num_clusters_list = list(range(2, 26, 1))
alpha_list = np.arange(0, 1.05, 0.05)
parallel_coordinates_plot(num_clusters_list, alpha_list)
