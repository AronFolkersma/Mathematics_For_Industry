...
def cor_metric(returns_df, p=0):
    ...
    if p==1:
        d = d = np.sqrt(0.5 * (1 - corr_matrix)).values
        temp = d.copy()
        for i in range(len(corr_matrix)):
          for j in range(len(corr_matrix)):
            temp[i][j]= np.sqrt(np.sum((d[:,i]-d[:,j])**2))
        return temp
...
def Cluster(p=0, alpha = 0, annual_returns_dict = annual_returns_dict, ESG_normalized = ESG_normalized, linkage_method = "ward", num_clusters = 6):
    ...
    global corr_dist_matrix; corr_dist_matrix = cor_metric(returns_df, p)
