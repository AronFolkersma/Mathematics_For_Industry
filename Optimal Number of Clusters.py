...
def collapse(tree):
    if tree["name"]: return [tree["name"]]                # leaf
...
def collapse_to_i_clusters(tree, i):
    ...
    return collapse_to_i_clusters(left) + collapse_to_i_clusters(right)
...
def dist_from_number_of_clusters(tree, i):
    ...
    if tree["clusters"] == i: return tree["dist"]

    left, right = tree["children"]

    return dist_from_number_of_clusters(left, i) + dist_from_number_of_clusters(right, i)

def optimal_number_of_clusters(matrix, distance_matrix, n:int, method:str, stocks=None):
    ...
    for n in ns:
        ...
        match method:
            ...
            case "elbow":
                vs[n-1] = sum(compute_wss(cluster, distance_matrix) for cluster in clusters)
            case "average sillhouette":
                vs[n-1] = silhouette_score(distance_matrix, y=clusters_to_labels(stocks, clusters), metric="precomputed")
